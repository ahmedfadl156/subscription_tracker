...
const ServiceLogo = ({ name, category }: { name: string; category: string }) => {
    ...
    if (logoUrl && !imgError) {
        return (
            <div
                className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center shrink-0 overflow-hidden"
                                ...
                                <Table.Row key={sub._id} id={sub._id}>
                                    ...
                                    <Table.Cell>
                                        <div className="flex items-center gap-3">
                                            <ServiceLogo name={sub.name} category={sub.category} />
                                            <div>
                                    ...
                                    {/* Actions */}
                                    <Table.Cell>
                                        <TableRowActionsDropdown isExpired={sub.status === "expired"} cancelUrl={sub?.cancelUrl || ""} subId={sub._id} />
                                    </Table.Cell>
